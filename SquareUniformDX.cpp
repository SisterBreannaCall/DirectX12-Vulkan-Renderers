#include "SquareUniformDX.h"

namespace DirectX
{
	void SquareUniformDX::createSquare(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
		vertices = {
	...
	void SquareUniformDX::createVertexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void SquareUniformDX::createIndexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void SquareUniformDX::createDescriptorHeap(ComPtr<ID3D12Device2> device)
	{
	...
	void SquareUniformDX::createUniformBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator)
	{
		size_t bufferSize = sizeof(PushConstBlock);
	...
	void SquareUniformDX::createGraphicsPipeline(ComPtr<ID3D12Device2> device)
	{
		ComPtr<ID3DBlob> vertShaderBlob;
		ComPtr<ID3DBlob> fragShaderBlob;

		D3DReadFileToBlob(L"data/shadersdx/DX2DUV.cso", &vertShaderBlob);
		...
		CD3DX12_ROOT_PARAMETER1 rootsParameters[1];
	...
	void SquareUniformDX::updateSquare()
	{
	...
	void SquareUniformDX::recordSquare(ComPtr<ID3D12GraphicsCommandList2> commandList, CD3DX12_CPU_DESCRIPTOR_HANDLE rtv, int width, int height)
	{
	...
	void SquareUniformDX::cleanUpSquare()
	{
	...
	void SquareUniformDX::reloadPipeline(ComPtr<ID3D12Device2> device)
	{
		createGraphicsPipeline(device);
	}
}