...
namespace DirectX
{
	void ModelDX::createModel(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void ModelDX::createVertexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void ModelDX::createIndexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void ModelDX::createDescriptorHeap(ComPtr<ID3D12Device2> device)
	{
	...
	void ModelDX::createUniformBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator)
	{
	...
	void ModelDX::createTexture(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
		auto commandList = queue->getCommandList(device);
	...
	void ModelDX::createGraphicsPipeline(ComPtr<ID3D12Device2> device)
	{
	...
	void ModelDX::updateModel(glm::mat4 viewMatrix, int width, int height)
	{
	...
	void ModelDX::recordModel(ComPtr<ID3D12GraphicsCommandList2> commandList, D3D12_CPU_DESCRIPTOR_HANDLE rtv, D3D12_CPU_DESCRIPTOR_HANDLE dsv, int width, int height)
	{
	...
	void ModelDX::reloadPipeline(ComPtr<ID3D12Device2> device)
	{
		createGraphicsPipeline(device);
	}
}