#include "LightCubeDX.h"

namespace DirectX
{
	void LightCubeDX::createCube(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue, UINT msaaSamples)
	{
		vertices = {
			{{-0.5f, -0.5f, -0.5f}},
			{{ 0.5f, -0.5f, -0.5f}},
	...
	void LightCubeDX::createVertexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void LightCubeDX::createDescriptorHeap(ComPtr<ID3D12Device2> device)
	{
	...
	void LightCubeDX::createUniformBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator)
	{
	...
	void LightCubeDX::createGraphicsPipeline(ComPtr<ID3D12Device2> device, UINT msaaSamples)
	{
	...
	void LightCubeDX::updateCube(glm::mat4 viewMatrix, int width, int height)
	{
	...
	void LightCubeDX::recordCube(ComPtr<ID3D12GraphicsCommandList2> commandList, D3D12_CPU_DESCRIPTOR_HANDLE rtv, D3D12_CPU_DESCRIPTOR_HANDLE dsv, int width, int height)
	{
	...
	void LightCubeDX::cleanUpCube()
	{
		uniformAllocation->Release();
		vertexAllocation->Release();
	}

	void LightCubeDX::reloadPipeline(ComPtr<ID3D12Device2> device, UINT msaaSamples)
	{
		if (enableWireframe)
		{
