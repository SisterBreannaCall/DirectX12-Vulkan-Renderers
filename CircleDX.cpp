...
namespace DirectX
{
	void CircleDX::createCircle(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
		// vertices
	...
	void CircleDX::createVertexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void CircleDX::createIndexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void CircleDX::createGraphicsPipeline(ComPtr<ID3D12Device2> device)
	{
		ComPtr<ID3DBlob> vertShaderBlob;
		ComPtr<ID3DBlob> fragShaderBlob;

		D3DReadFileToBlob(L"data/shadersdx/DX2DCV.cso", &vertShaderBlob);
	...
	void CircleDX::recordCircle(ComPtr<ID3D12GraphicsCommandList2> commandList, CD3DX12_CPU_DESCRIPTOR_HANDLE rtv, int width, int height)
	{
		...
		commandList->SetGraphicsRoot32BitConstants(0, sizeof(PushConstBlock) / 4, &pushConstBlock, 0);
	...
	void CircleDX::cleanUpCircle()
	{
		indexAllocation->Release();
		vertexAllocation->Release();
	}

	void CircleDX::reloadPipeline(ComPtr<ID3D12Device2> device)
	{
		if (enableWireframe)
		{
