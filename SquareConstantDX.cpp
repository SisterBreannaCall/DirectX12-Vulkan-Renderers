#include "SquareConstantDX.h"

namespace DirectX
{
	void SquareConstantDX::createSquare(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
		vertices = {
	...
	void SquareConstantDX::createVertexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void SquareConstantDX::createIndexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void SquareConstantDX::createGraphicsPipeline(ComPtr<ID3D12Device2> device)
	{
	...
	void SquareConstantDX::recordSquare(ComPtr<ID3D12GraphicsCommandList2> commandList, CD3DX12_CPU_DESCRIPTOR_HANDLE rtv, int width, int height)
	{
	...
	void SquareConstantDX::cleanUpSquare()
	{
		indexAllocation->Release();
		vertexAllocation->Release();
	}

	void SquareConstantDX::reloadPipeline(ComPtr<ID3D12Device2> device)
	{
		createGraphicsPipeline(device);
	}
}