...
namespace DirectX
{
	void TriangleDX::createTriangle(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
		vertices = {
	...
	void TriangleDX::createVertexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void TriangleDX::createIndexBuffer(ComPtr<ID3D12Device2> device, D3D12MA::Allocator* allocator, std::shared_ptr<QueueDX> queue)
	{
	...
	void TriangleDX::createGraphicsPipeline(ComPtr<ID3D12Device2> device)
	{
	...
	void TriangleDX::recordTriangle(ComPtr<ID3D12GraphicsCommandList2> commandList, CD3DX12_CPU_DESCRIPTOR_HANDLE rtv, int width, int height)
	{
	...
	void TriangleDX::cleanUpTriangle()
	{
		indexAllocation->Release();
		vertexAllocation->Release();
	}

	void TriangleDX::reloadPipeline(ComPtr<ID3D12Device2> device)
	{
		createGraphicsPipeline(device);
	}
}